...
month_subsets = {
    ...
    "1-6": list(range(1, 6+1)), "7-12": list(range(7, 12+1)), 
    "all": list(range(1, 12+1))
}
months_to_plot_valid = ["1-6", "7-12"]
hour_subsets = {
    "0-5": list(range(0, 5+1)), "6-11": list(range(6, 11+1)),
    "12-17": list(range(12, 17+1)), "18-23": list(range(18, 23+1)),
    "all": list(range(0, 23+1))
}
hours_to_plot_valid = ["0-5", "6-11", "12-17", "18-23"]
...
params_stat_vector = ["max_u", "max_v", "min_u", "min_v", "mean_u", "mean_v"]

# Output parameters for calc_era5_mean_clim_given_var_or_dvar
params_stat_mean = ["mean", "mean_u", "mean_v"]
...
arg_extra_all = params_glass_mean + hour_subsets["all"] + params_stat + params_wsd
...
def check_args_for_none(func_name, args_1up=None, args_1up_values=None):
    ...
    if func_name in ["convert_period_data_types",
                     "calc_glass_mean_clim",
                     "calc_era5_mdp_clim_given_var_or_dvar", 
                     "calc_era5_mdp_clim_stats_given_var_or_dvar"]:
        for arg in ["period_hours", "period1_hours", "period2_hours"]:
            try:
                assert args_1up_values[arg] == None, \
                    f"{arg} must be None if calc_func = {func_name}"
                args_1up.remove(arg)
            except:
                pass
...
def check_args(
    calc_func=None, region=None, 
    period_start=None, period_end=None, period_months=None, period_hours=None,
    period1_start=None, period1_end=None, period2_start=None, period2_end=None, 
    period1_months=None, period2_months=None, period1_hours=None, period2_hours=None,
    ...
    param_glass_mean=None, var_or_dvar_layer=None, var_or_dvar_type=None, 
    hours_to_plot=None, months_to_plot=None, perc=None, mask_perc_quantile=None, 
    ...
    if period_months:
        assert (isinstance(period_months, list) & (period_months != []) & 
                all(month in month_subsets["all"] for month in period_months)
               ) | (period_months in [*month_subsets]), \
            (f"period_months must be type str and one of: {[*month_subsets]}, or type " +
             "list and subset of: {} with at least one item".format(month_subsets["all"]))
        
    if period1_months:
        assert (isinstance(period1_months, list) & (period1_months != []) & 
                all(month in month_subsets["all"] for month in period1_months)
               ) | (period1_months in [*month_subsets]), \
            (f"period1_months must be type str and one of: {[*month_subsets]}, or type " +
             "list and subset of: {} with at least one item".format(month_subsets["all"]))
        
    if period2_months:
        assert (isinstance(period2_months, list) & (period2_months != []) & 
                all(month in month_subsets["all"] for month in period2_months)
               ) | (period2_months in [*month_subsets]), \
            (f"period2_months must be type str and one of: {[*month_subsets]}, or type " +
             "list and subset of: {} with at least one item".format(month_subsets["all"]))
    ...
    if (period_start is not None) & (period_end is not None) & (period_months is not None):
        ...
        if isinstance(period_months, str):
            period_months = month_subsets[period_months]
    ...
    if (period1_start is not None) & (period1_end is not None) & (period1_months is not None):
        ...
        if isinstance(period1_months, str):
            period1_months = month_subsets[period1_months]
    ...
    if (period2_start is not None) & (period2_end is not None) & (period2_months is not None):
        ...
        if isinstance(period2_months, str):
            period2_months = month_subsets[period2_months]
    ...
    if period_hours:
        assert (isinstance(period_hours, list) & (period_hours != []) & 
                all(hour in hour_subsets["all"] for hour in period_hours)
               ) | (period_hours in [*hour_subsets]), \
            (f"period_hours must be type str and one of: {[*hour_subsets]}, or type " +
             "list and subset of: {} with at least one item".format(hour_subsets["all"]))
        
    if period1_hours:
        assert (isinstance(period1_hours, list) & (period1_hours != []) & 
                all(hour in hour_subsets["all"] for hour in period1_hours)
               ) | (period1_hours in [*hour_subsets]), \
            (f"period1_hours must be type str and one of: {[*hour_subsets]}, or type " +
             "list and subset of: {} with at least one item".format(hour_subsets["all"]))
        
    if period2_hours:
        assert (isinstance(period2_hours, list) & (period2_hours != []) & 
                all(hour in hour_subsets["all"] for hour in period2_hours)
               ) | (period2_hours in [*hour_subsets]), \
            (f"period2_hours must be type str and one of: {[*hour_subsets]}, or type " +
             "list and subset of: {} with at least one item".format(hour_subsets["all"]))
    ...
    if (arg_extra is not None) & (calc_func is not None):
        ...
        if calc_func_name == "calc_era5_mdp_clim_given_var_or_dvar":
            assert arg_extra in hour_subsets["all"], \
                ("arg_extra must be one of: {} ".format(hour_subsets["all"]) +
                 f"for calc_func = {calc_func_name}")
        if calc_func_name == "calc_era5_mdp_clim_stats_given_var_or_dvar":
            assert arg_extra in params_stat, \
                (f"arg_extra must be one of: {params_stat} " +
                 f"for calc_func = {calc_func_name}")
            if (arg_extra in params_stat_vector) & (var_or_dvar is not None):
                assert var_or_dvar in params_vector, \
                    (f"var_or_dvar must be one of: {params_vector} " +
                     f"for arg_extra = {arg_extra}")
        if calc_func_name == "calc_era5_mean_clim_given_var_or_dvar":
            assert arg_extra in params_stat_mean, \
                (f"arg_extra must be one of: {params_stat_mean} " +
                 f"for calc_func = {calc_func_name}")
            if (arg_extra in params_stat_vector) & (var_or_dvar is not None):
                assert var_or_dvar in params_vector, \
                    (f"var_or_dvar must be one of: {params_vector} " +
    ...
    if hours_to_plot:
        assert hours_to_plot in hours_to_plot_valid, \
            f"hours_to_plot must be one of: {hours_to_plot_valid}"
        
    if months_to_plot:
        assert months_to_plot in months_to_plot_valid, \
            f"months_to_plot must be one of: {months_to_plot_valid}"
...
def get_period_hours_str(period_hours):
    
    """
    Obtain the string representation of period_hours for use in output paths.
    ...
    Returns:
        period_hours_str (str): String representation of period_hours.
        
    If period_hours is one of the string inputs then this function does nothing. But
    if period_hours is a list input, this function concatenates the elements of the
    list into a string joined by "-" and in ascending numerical order.
    """
    ...
    check_args_for_none(func_cur, args_cur, args_cur_values)
    check_args(period_hours=period_hours)
    ...
    if (func_1up == "<cell line: 1>") | (func_1up == "<module>"):
        ...
        logging.debug(f"Executing: {func_cur} to obtain string representation of " +
                      f"{period_hours} for use in {func_1up}.")
    
    if isinstance(period_hours, str):
        period_hours_str = period_hours
    
    if isinstance(period_hours, list):
        period_hours.sort()
        period_hours_str = "-".join(str(hour) for hour in period_hours)
        
    if (func_1up == "<cell line: 1>") | (func_1up == "<module>"):
        logging.info(f"Obtained: string representation of {period_hours}.")
    else:
        logging.info(f"Obtained: string representation of {period_hours} " +
                     f"for use in {func_1up}.")
    
    remove_handlers_if_directly_executed(func_1up)
    return period_hours_str


# In[ ]:


def convert_period_data_types(period_start, period_end, period_months, period_hours=None):
    ...
    Returns:
        ...
        period_hours (list): Hours subset of period to perform calculation over 
            (returned only if not None).
    ...
    season then it will be converted to a list with the months in that subset. If 
    period_hours is a list then there is no conversion but if it is a string specifying 
    "all" or a subset then it will be converted to a list with the hours in that subset.
    """
    ...
    if period_hours:
        if isinstance(period_hours, str):
            period_hours = hour_subsets[period_hours]
    ...
    if period_hours:
        return period_start, period_end, period_months, period_hours
    else:
...
def get_path_for_calc_func(calc_func_name, region, period_start, period_end, 
                           period_months, period_hours=None, glass_source_pref=None,
                           var_or_dvar=None):
    ...
    # Assert that input arguments are valid, and create path for periods.
    ...
    if calc_func_name == "calc_era5_mean_clim_given_var_or_dvar":
        period_hours_str = get_period_hours_str(period_hours=period_hours)
        path_output_calc_func += f"era5-mean_{period_hours_str}_{var_or_dvar}.nc"
        
    if calc_func_name == "calc_era5_wsd_clim":
        period_hours_str = get_period_hours_str(period_hours=period_hours)
        path_output_calc_func += f"era5-wsd_{period_hours_str}.nc"
...
def get_path_for_calc_diff(
    calc_func_name, region, period1_start, period1_end, period2_start, period2_end, 
    period1_months, period2_months, period1_hours=None, period2_hours=None, 
    ...
    if calc_func_name == "calc_era5_mean_clim_given_var_or_dvar":
        period1_hours_str = get_period_hours_str(period_hours=period1_hours)
        period2_hours_str = get_period_hours_str(period_hours=period2_hours)
        path_output_calc_diff += (f"era5-mean_{period1_hours_str}_" + 
                                  f"{period2_hours_str}_{var_or_dvar}.nc")
        
    if calc_func_name == "calc_era5_wsd_clim":
        period1_hours_str = get_period_hours_str(period_hours=period1_hours)
        period2_hours_str = get_period_hours_str(period_hours=period2_hours)
        path_output_calc_diff += f"era5-wsd_{period1_hours_str}_{period2_hours_str}.nc"
...
def calc_glass_mean_clim(region, period_start, period_end, period_months, 
                         glass_source_pref, period_hours=None, var_or_dvar=None):
    ...
    Arguments:
        ...
        glass_source_pref (str): Preferred glass data source to use when analysis is 
            over a period which is completely contained within both the available
            AVHRR and MODIS datasets. Must be one of: ["avhrr", "modis"].
        period_hours (None): This argument is not used for this analysis. It is used 
    ...
    # computing using dask. The region and hour_subsets (period with period_months) are
...
def calc_era5_mdp_clim_given_var_or_dvar(region, period_start, period_end, 
                                         period_months, var_or_dvar, period_hours=None,
    ...
    if Path(path_output_var).exists():
        ...
        ds_era5_mdp = (ds_era5_mdp
                       ...
                       # have perfect alignment across all variables, and somehour_subsets
...
def calc_era5_mdp_clim_stats_given_var_or_dvar(region, period_start, period_end, 
                                               ...
                                               period_hours=None, glass_source_pref=None):
    """
    Calculate the mean diurnal profile (MDP) climatology statistics for a particular
    variable or change in variable (compared to previous hour) using ERA5 data.
...
def calc_era5_mean_clim_given_var_or_dvar(region, period_start, period_end, 
                                          period_months, period_hours, var_or_dvar, 
                                          glass_source_pref=None):
    
    """
    Calculate the mean climatology statistics for a particular
    variable or change in variable (compared to previous hour) using ERA5 data.
    ...
    Returns:
        ../data_processed/era5_mean_clim_given_var_or_dvar/{calc_funcs_ver}_calc_
        {region}_{period_start}_{period_end}_{period_months_str}_{period_hours_str}_
        era5-mean_{var_or_dvar}.nc:
            Output netcdf4 file in data_processed folder containing mean values
            for given var_or_dvar over the selected months and hours subsets.
            {calc_funcs_ver} is the version of the calc_funcs script
            being used. {period_months_str} is a string representing the list of 
            selected months to use as a subset. {period_hours_str} is a string 
            representing the list of selected hours to use as a subset.
    
    For each grid cell, calculate the mean of the selected var_or_dvar over the selected
    period and month and hour subset.. The mean values are computed over the period 
    from period_start to period_end (inclusive), and only using a subset of data within 
    this period (if period_months and/or period_hours not "all" is specified). The 
    calculation uses monthly averaged reanalysis by hour of day netcdf4 data from the 
    data_raw folder as input, then outputs the result as a netcdf4 file into the 
    data_processed folder.
    """
    ...
    # Assert that input arguments are valid, and create path for period.
    ...
    if (func_1up == "<cell line: 1>") | (func_1up == "<module>"):
        logging.info(f"Executing: {func_cur} to obtain mean climatology of " +
                     f"{var_or_dvar} between {period_start} and {period_end} " +
                     f"(months={period_months_str}, hours={period_hours_str}).")
    else:
        logging.info(f"Executing: {func_cur} to obtain mean climatology of " +
                     f"{var_or_dvar} between {period_start} and {period_end} " +
                     f"(months={period_months_str}, hours={period_hours_str}) " +
    ...
    path_output_mean_clim = get_path_for_calc_func(
        calc_func_name=func_cur, region=region, period_start=period_start, 
        period_end=period_end, period_months=period_months, period_hours=period_hours,
        var_or_dvar=var_or_dvar)
    terminate_if_file_exists(path_output_mean_clim, func_cur, func_1up)
    ...
    # Convert period_start and period_end to datetime.datetime objects, 
    # and period subsets to list if a str was used as input.
    
    period_start, period_end, period_months, period_hours = convert_period_data_types(
        period_start=period_start, period_end=period_end, 
        period_months=period_months, period_hours=period_hours)
    
    # Compute mean for given var_or_dvar, treating vector values separately.
    logging.debug(f"Computing: {var_or_dvar} mean for use in {func_cur}.")    
    if var_or_dvar in params_vector:
        if var_or_dvar in vars_era5_all:
            var_or_dvar_attrs = copy.deepcopy(attrs_da[var_or_dvar])
        if var_or_dvar in dvars_era5_all:
        ...
        if priority == "speed":
            ds_era5_mdp = ds_era5_mdp.persist()
        da_u = (ds_era5_mdp[var_or_dvar.replace("wv", "u")]
                .sel(hour = ds_era5_mdp.hour.isin(period_hours)))
        da_v = (ds_era5_mdp[var_or_dvar.replace("wv", "v")]
                .sel(hour = ds_era5_mdp.hour.isin(period_hours)))
        da_mean_u = xr.DataArray(da_u.mean("hour"), name = "mean_u")
        da_mean_v = xr.DataArray(da_v.mean("hour"), name = "mean_v")
        ds_era5_mean = xr.merge([da_mean_u, da_mean_v])
    else:
        da_era5_mdp = xr.open_dataset(path_era5_mdp, engine = "netcdf4")[var_or_dvar]
        var_or_dvar_attrs = copy.deepcopy(da_era5_mdp.attrs)
        if priority == "speed":
            da_era5_mdp = da_era5_mdp.persist()
        ds_era5_mean = (da_era5_mdp
                        .sel(hour = da_era5_mdp.hour.isin(period_hours))
                        .mean("hour")
    ...
    logging.info("Adding: attributes for each DataArray within output "+
                 f"Dataset from {func_cur}.")
    for da_name in [*ds_era5_mean.keys()]:
        ds_era5_mean[da_name].attrs = copy.deepcopy(attrs_da[da_name])
        ds_era5_mean[da_name].attrs["abbreviation"] = (
            ds_era5_mean[da_name].attrs["abbreviation"]
            .format(var_or_dvar_attrs["abbreviation"])
        )
        ds_era5_mean[da_name].attrs["full_name"] = (
            ds_era5_mean[da_name].attrs["full_name"]
            .format(var_or_dvar_attrs["full_name"])
        )
        ds_era5_mean[da_name].attrs["units"] = (
            ds_era5_mean[da_name].attrs["units"]
            .format(var_or_dvar_attrs["units"])
        )
    ...
    add_ds_attrs(ds_era5_mean, time_exec, func_cur, func_1up, 
                 args_cur, args_cur_values)
    
    # Create output file in data_processed folder.
    
    create_output_data_file(ds_era5_mean, path_output_mean_clim, func_1up)
    remove_handlers_if_directly_executed(func_1up)
...
def calc_era5_wsd_clim(region, period_start, period_end, period_months, period_hours,
                       glass_source_pref=None, var_or_dvar=None):
    ...
    Returns:
        ...
        {period_end}_{period_months_str}_{period_hours_str}_era5-wsd.nc:
            ...
            to use as a subset. {period_hours_str} is a string representing the list 
            of selected hours to use as a subset.
    ...
    (if a period_months and/or period_hours not "all" is specified). The calculation 
    uses hourly ERA5 netcdf4 data from the data_raw folder as input, then outputs the 
    result as a netcdf4 file into the data_processed folder.
    """
    ...
    if (func_1up == "<cell line: 1>") | (func_1up == "<module>"):
        logging.info(f"Executing: {func_cur} to obtain wind speed distribution " +
                     f"parameters over {period_months_str} months and " + 
                     f"{period_hours_str} hours between {period_start} and " +
                     f"{period_end}.")
    else:
        logging.info(f"Executing: {func_cur} to obtain wind speed distribution " +
                     f"parameters over {period_months_str} months and " + 
                     f"{period_hours_str} hours between {period_start} and " +
                     f"{period_end} for use in {func_1up}.")
    
    # Define the output path, convert period_start and period_end to
    # datetime.datetime objects, and period subsets to list if a str was used as input.
    ...
    period_start, period_end, period_months, period_hours = convert_period_data_types(
        period_start=period_start, period_end=period_end, 
        period_months=period_months, period_hours=period_hours)
    period_hours_utc = [(hour - regions[region]["tz"]) % 24 for hour in period_hours]
    ...
    def filter_era5_hour_files(file_name):
        ...
        # filter function later, in order to select out the raw data files with
        # years within the input period. The following preprocess function also
        # selects out the relevant years (as well as months and hours) but by 
        # applying a filter on the list of file names first we can avoid  
        # preprocessing a lot of files and hence save on memory.
        year = int(file_name[-7:-3])
        if period_start.year <= year <= period_end.year:
            return True
        else:
    ...
    def preprocess_era5_hour(ds):
        ...
        # the subset months and hours. This is done for persist scalability.
        file_name = ds.encoding["source"]
        logging.debug(f"Preprocessing: file for use in {func_cur}: {file_name}.")
        ds = (regrid_era5(ds=ds)[["u10", "v10", "u100", "v100"]]
              .sel(time = ds.time.dt.month.isin(period_months))
             )
        ds = (ds.sel(time = ds.time.dt.hour.isin(period_hours_utc))
              # The downloaded ERA5 dataset is not sorted in time order (which
              # is a necessity for open_mfdataset, so we sort first over here.
              .sortby("time")
             )
...
def calc_diff(calc_func, region, period1_start, period1_end,
              ...
              period1_hours=None, period2_hours=None,
              glass_source_pref=None, var_or_dvar=None):
    ...
    Returns:
        ...
        ../data_processed/era5_mean_clim_given_var_or_dvar/{calc_funcs_ver}_diff_
            {region}_{period1_start}_{period1_end}_{period2_start}_{period2_end}_
            {period1_months_str}_{period2_months_str}_{period1_hours_str}_
            {period2_hours_str}_era5-mean_{var_or_dvar}.nc OR
        ../data_processed/era5_wsd_clim/{calc_funcs_ver}_diff_{region}_{period1_start}_
            {period1_end}_{period2_start}_{period2_end}_{period1_months_str}_
            {period2_months_str}_{period1_hours_str}_{period2_hours_str}_era5-wsd.nc:
                ...
                {period1_hours_str} is a string representing the list of selected 
                hours to use as a subset for period1. {period2_hours_str} is a string 
                representing the list of selected hours to use as a subset for period2. 
    ...
    calc_func_name = calc_func.__name__
    check_args_for_none(calc_func_name, args_cur, args_cur_values)
    check_args(calc_func=calc_func, region=region, 
...
def calc_glass_rolling_avg_of_annual_diff(region, year_start, year_end, period_months,
    ...
    for year in range(int(year_start - (window_size-1)/2 + 1), 
        ...
        path_diff = get_path_for_calc_diff(
            ...
            period1_months=period_months, period2_months=period_months, 
            glass_source_pref=glass_source_pref)
...
def create_all_possible_calc_data_files(region, period_start, period_end, 
                                        period_months, period_hours, glass_source_pref):
    ...
    if (func_1up == "<cell line: 1>") | (func_1up == "<module>"):
        logging.info(f"Executing: {func_cur} to create all possible calc data files " +
                     f"in {region} between {period_start} and {period_end} " + 
                     f"(months={period_months_str}, hours={period_hours_str}).")
    else:
        logging.info(f"Executing: {func_cur} to create all possible calc data files " +
                     f"in {region} between {period_start} and {period_end} " + 
    ...
    period_hours_input = copy.deepcopy(period_hours)
    glass_source_pref_input = copy.deepcopy(glass_source_pref)
    ...
    for calc_func_name in calc_func_names:
        ...
        if calc_func_name != "calc_glass_mean_clim":
            ...
            glass_source_pref = copy.deepcopy(glass_source_pref_input)
        
        if calc_func_name in ["calc_glass_mean_clim",
                              "calc_era5_mdp_clim_given_var_or_dvar", 
                              "calc_era5_mdp_clim_stats_given_var_or_dvar"]:
            period_hours = None
        else:
            period_hours = copy.deepcopy(period_hours_input)
        
        if calc_func_name in ["calc_glass_mean_clim", "calc_era5_wsd_clim"]:
...
def create_all_possible_diff_data_files(
    ...
    if (func_1up == "<cell line: 1>") | (func_1up == "<module>"):
        logging.info(f"Executing: {func_cur} to create all possible difference " +
                     f"data files in {region} over {period1_start} to {period1_end} " +
                     f"(months={period1_months_str}, hours={period1_hours_str}) " + 
                     f"and {period2_start} to {period2_end} " +
                     f"(months={period2_months_str}, hours={period2_hours_str}).")
    else:
        logging.info(f"Executing: {func_cur} to create all possible difference " +
                     f"data files in {region} over {period1_start} to {period1_end} " +
                     f"(months={period1_months_str}, hours={period1_hours_str}) " + 
    ...
    period1_hours_input = copy.deepcopy(period1_hours)
    period2_hours_input = copy.deepcopy(period2_hours)
    glass_source_pref_input = copy.deepcopy(glass_source_pref)
    ...
    for calc_func_name in calc_func_names:
        ...
        if calc_func_name in ["calc_glass_mean_clim",
            ...
            period1_hours = copy.deepcopy(period1_hours_input)
            period2_hours = copy.deepcopy(period2_hours_input)
